export interface Service {
  id: string;
  title: string;
  description: string;
  bullets: string[];
  icon: string; // Simple SVG path data for an inline icon
}

export const services: Service[] = [
  {
    id: 'content-creation',
    title: 'Content Creation',
    description:
      'Content that gets attention and drives action. We create high-impact visuals built for TikTok, Instagram and YouTube.',
    bullets: [
      'Liveshots and multicam recording',
      'Event films and aftermovies',
      'Artist promos and brand visuals',
      'Short-form content that performs',
    ],
    icon: 'M15 10l4.553-2.277A1 1 0 0 1 21 8.618v6.764a1 1 0 0 1-1.447.894L15 14v-4zm-2 7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v10z',
  },
  {
    id: 'social-media-management',
    title: 'Social Media Management',
    description:
      'We run your socials. You focus on your work. Everything is built around growth, consistency and results.',
    bullets: [
      '2 to 10 posts per week',
      'Content planning and strategy',
      'Instagram, TikTok and YouTube',
      'Community and engagement',
      'Monthly performance tracking',
      'Reel sequencing and growth loops',
    ],
    icon: 'M4 5a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5zm0 6a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-8zm10 0a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1v-4zm0 7a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1v-1z',
  },
  {
    id: 'web-design',
    title: 'Web Design & Strategy',
    description:
      'Websites built to convert. Turn traffic into bookings, leads and customers.',
    bullets: [
      'Custom design and development',
      'Mobile-first and fast',
      'SEO and performance',
      'Landing pages and campaigns',
      'Domain and hosting handled',
      'Shipped fast. Fully yours.',
    ],
    icon: 'M3 3h18a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm6 18h6m-3-4v4',
  },
];
